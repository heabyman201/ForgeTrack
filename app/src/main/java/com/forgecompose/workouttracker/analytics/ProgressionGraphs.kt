...
@Composable
fun ExerciseAverageHrProgressionGraph(
    exerciseName: String,
    workouts: List<Workout>
) {
    val exerciseData = remember(workouts, exerciseName) {
        workouts
            .filter { it.name == exerciseName && (it.heartRateAvg ?: 0) > 0 }
            .sortedBy { it.date }
    }

    val maxAvgHr = remember(exerciseData) { exerciseData.maxOfOrNull { it.heartRateAvg ?: 0 } ?: 0 }
    val minAvgHr = remember(exerciseData) { exerciseData.minOfOrNull { it.heartRateAvg ?: 0 } ?: 0 }
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text(
                "Avg HR Progression for $exerciseName",
                style = MaterialTheme.typography.headlineSmall,
                fontWeight = FontWeight.Bold,
                color = Color.White
            )
            ...
            if (exerciseData.size < 2) {
                ...
                LineChartAverageHr(
                    data = exerciseData,
                    maxValue = maxAvgHr.toDouble(),
                    minValue = minAvgHr.toDouble()
                )
            }
        }
    }
